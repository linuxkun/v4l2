...
#define IMAGE_WIDTH 800//sensor固定输出1920*1080的图像
...
int cam_init()
{
	...
	format.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;//YUV格式
    ...
    if(ioctl(cam_fd,VIDIOC_G_FMT,&format) != 0) {
        DBG("ioctl(VIDIOC_G_FMT) failed %d(%s)", errno, strerror(errno));
        return ret;
    }
    
    printf("width = %d height = %d format = %s\n",format.fmt.pix.width = IMAGE_WIDTH,format.fmt.pix.height = IMAGE_HEIGHT,
                                                format.fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV?"V4L2_PIX_FMT_YUYV":"NULL");
...
int main()
{
    ...
    while (1)
    {
        ...
        FILE *fd = fopen(filename,"w+");//保存图像数据
        if (fd >= 0)
        {
            fwrite(buf, 1,IMAGE_SIZE,fd);
            fclose(fd);
        }
        else
        {
